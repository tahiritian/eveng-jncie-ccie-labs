...
VARIABLES_FILE = 'isp-variables.yml'
...
def configure_device(connection_params, variables):
    ...
    with Config(device_connection, mode='private') as config:
        config.load(template_path='isp-j2.conf', template_vars=config_variables, format='text', merge=True)
        print("Config diff:")
        config.pdiff()
        config.commit()
