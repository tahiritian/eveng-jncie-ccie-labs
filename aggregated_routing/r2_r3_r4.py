...
VARIABLES_FILE = 'r2_r3_r4-variables.yml'
...
def configure_device(connection_params, variables):
    ...
    with Config(device_connection, mode='private') as config:
        config.load(template_path='r2_r3_r4_j2.conf', template_vars=config_variables, format='text', merge=True)
        print("Config diff:")
        config.pdiff()
        config.commit()
...
def get_devices_ip_addresses():
    return read_yaml()['juniper-r2-r3-r4']
